function y = vl_nnpdist_pzy(x, x0, p, varargin)
...
%       y = sum(abs(d),3) ;
    if p == 1
      y = sum(abs(d),4) ;%计算视差Loss项
      %为避免出现无穷大值，直接采用SAD，绝对误差和公式
%     elseif p == 2
%       y = sum(d.*d,3) ;
    elseif p == 2
      y = sum(d.*d,4) ;
    ...
    if opts.aggregate
      y = sum(sum(y))/(H*W*N) ;%每一个batch中的所有子图像（H*W）的总误差，若是求每个像素的误差（y/（H*W*N））
    end
  else
%     if p == 1
%       y = bsxfun(@times, dzdy, sign(d)) ;
    if p == 1
      y = bsxfun(@times, dzdy, sign(d)) ;%计算视差Loss项，
      %采用绝对误差和，SAD的求导
    elseif p == 2
