function res = vl_simplenn_3D_differ_pzy(net, x, dzdy, res, varargin)
...
% 自己新增的loss函数-----------------------------------------------------------2016.11.9
    case 'MSEloss'
      res(i+1).x = vl_nnpdist_pzy(res(i).x, l.class,2,'noRoot',true,'aggregate',true) ;
% 自己新增的loss函数-----------------------------------------------------------2016.12.18
    case 'SAD_loss'
      res(i+1).x = vl_nnpdist_pzy(res(i).x, l.class,1,'noRoot',true,'aggregate',true) ;
%-----------------------------------------------------------------------------     
    case 'relu'
...
% 自己新增的loss函数-----------------------------------------------------------2016.11.9  
      case 'MSEloss'
        res(i).dzdx = vl_nnpdist_pzy(res(i).x, l.class,2, res(i+1).dzdx,'noRoot',true,'aggregate',true) ;
% 自己新增的loss函数-----------------------------------------------------------2016.12.18  
      case 'SAD_loss'
        res(i).dzdx = vl_nnpdist_pzy(res(i).x, l.class,1, res(i+1).dzdx,'noRoot',true,'aggregate',true) ;        
%-----------------------------------------------------------------------------         
      case 'relu'
